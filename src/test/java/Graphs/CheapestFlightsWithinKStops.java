...
public class CheapestFlightsWithinKStops
{
    ...
    public int findCheapestPrice_brute_force(int n, int[][] flights, int src, int dst, int k)
    {
        maxStops = k;
        connections = new int[n][n];

        for (int [] flight : flights)
            connections[flight[0]][flight[1]] = flight[2];

        int cost  = search(src, dst, 0);

        return (cost == Integer.MAX_VALUE) ? -1 : cost;
    }

    public int search(int from, int dst, int stops)
    {
        int selectedPathPrice = 0;
        ...
        {
            if (connections[from][i] > 0 )
            {
                int price = 0;

                if (i == dst) // reached the final destination
                    price = connections[from][i];
                else
                    price = search(i, dst, stops) == Integer.MAX_VALUE ? Integer.MAX_VALUE
                            : connections[from][i] + search(i, dst, stops);
    ...
    public int findCheapestPrice_bellman_ford(int n, int[][] flights, int src, int dst, int k)
    {
        ...
        while ( k-- != -1 )
        {
            ...
            {
                int source = edge[0], destination = edge[1], price = edge[2];

                if (prices[source] == Integer.MAX_VALUE ) // haven't reached this edge yet, looping further
                    continue;
    ...
    public void test()
    {
        ...
        assert (700 == findCheapestPrice_brute_force(4, flights1, 0,3,1));
        assert (700 == findCheapestPrice_bellman_ford(4, flights1, 0,3,1));

        int [][] flights2 = {{0,1,100},{1,2,100},{0,2,500}};
        assert (200 == findCheapestPrice_brute_force(3, flights2, 0,2,1));
        assert (200 == findCheapestPrice_bellman_ford(3, flights2, 0,2,1));

        assert (500 == findCheapestPrice_brute_force(3, flights2, 0,2,0));
        assert (500 == findCheapestPrice_bellman_ford(3, flights2, 0,2,0));

        int [][] flights3 = {{0,12,28},{5,6,39},{8,6,59},{13,15,7},{13,12,38},{10,12,35},{15,3,23},{7,11,26},{9,4,65}
        ...
        assert (47 == findCheapestPrice_bellman_ford(17, flights3, 13,4,13));
        //assert (47 == findCheapestPrice_brute_force(17, flights3, 13,4,13));
    }
}
